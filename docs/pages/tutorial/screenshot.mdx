...
In the above snippet, the `collapsable` prop is set to `false`. This allows the `<View>` component to take a screenshot only of the background image and emoji sticker.
